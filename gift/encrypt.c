...
/****************************************************************************
* Permutation of 64-bit blocks. Note that 'ptext_len' must be a mutliple of 8.
****************************************************************************/
int gift64_perm_ecb(u8* ctext, const u8* ptext, u32 ptext_len) {
	u32 state[4], rkey[56];
	memset(rkey, 0x0, 56 * sizeof(u32));
	// while(ptext_len > BLOCK_SIZE) {		// Processing 2 blocks at once
	// 	packing(state, ptext, ptext + BLOCK_SIZE);
